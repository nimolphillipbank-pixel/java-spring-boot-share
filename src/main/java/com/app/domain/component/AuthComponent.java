package com.app.domain.component;
...
public class AuthComponent {
  ...
  public AuthComponent(CredentialProperty credentialProperty) {
    ...
    if (!StringUtils.hasText(this.credentialProperty.getClientId())) {
      throw new IllegalStateException(
          "Missing required property 'credentials.client-id'.");
    }
    if (!StringUtils.hasText(this.credentialProperty.getSecretKey())) {
      throw new IllegalStateException(
          "Missing required property 'credentials.secret-key'.");
    }
    if (!StringUtils.hasText(this.credentialProperty.getHmacSecret())) {
      throw new IllegalStateException(
          "Missing required property 'credentials.hmac-secret'. Configure an active profile "
              + "that provides credentials, for example 'local,db_local,cache_local'.");
    }
